...
Target runtime: **4:55**. Record at 1080p. Keep the persistent `TEST MODE · NO REAL MONEY` banner visible.
...
## 0:35–1:45 — Live Recovery Demo

Open **Live Demo** and choose **Start end-to-end demo**. A fresh signed failure appears, then the visible agent trace advances through evidence diagnosis, four counterfactual futures, adversarial challenge, deterministic policy, and one recovery action. Show the cause, confidence, natural-recovery baseline, action success probability, conservative value, and guardrail result. Choose **Simulate customer completing recovery** and show the signed outcome move the case to `RECOVERED`.

If Razorpay Test Mode credentials and a public webhook are configured, use **Open Razorpay Test Checkout** instead, intentionally fail the hosted payment, and wait for the provider-signed webhook. Never describe the signed guided mode as a provider-delivered event.

## 1:45–2:15 — Recovery Inbox
...
## 4:35–4:55 — Architecture and close
