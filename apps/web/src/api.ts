...
export const api = {
  ...
  execute: (id: string): Promise<{ executed: boolean; reason: string; state?: string; mode_label?: string }> => fetch(`/api/v1/cases/${id}/execute`, { method: 'POST' }).then(json<{ executed: boolean; reason: string; state?: string; mode_label?: string }>),
